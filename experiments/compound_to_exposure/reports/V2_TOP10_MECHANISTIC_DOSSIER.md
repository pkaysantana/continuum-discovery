...
1. **Hypothesis that basic/lipophilic chemistry may contribute to some low-clearance overpredictions via microsomal binding**: The model's reliance on 2D fragments fails to capture global physicochemical properties like basicity and lipophilicity that drive high non-specific microsomal binding (low fu,mic). This binding reduces free drug availability in vitro, potentially contributing to the overpredictions observed for lipophilic bases.
2. **Several high-clearance underpredictions contain motifs plausibly susceptible to metabolism**: The model underestimates the clearance of compounds containing classic, highly vulnerable metabolic liabilities (e.g., exposed thioethers, terminal methoxy groups, thiophenes) when those local structural contexts differ sufficiently from the training data.
